...
function flowRow(keys, gap) {
  ...
  keys.forEach((key, index) => {
    ...
    nodes.push({ key, art, index: nodes.length, x: cursor, w: art.width * depth, h: art.height * depth });
...
function buildScene(scene) {
  ...
  return [
    ...back.map((node) => describe(node, { origin: BACK_ORIGIN, fit: backFit, fitCompact: backFitCompact, row: "back", delayBase: OPEN_ITEM_BASE - 40, delayStep: 70, from: 0.9 })),
    ...front.map((node) => describe(node, { origin: FRONT_ORIGIN, fit: frontFit, fitCompact: frontFitCompact, row: "front", delayBase: OPEN_ITEM_BASE, delayStep: OPEN_ITEM_STAGGER, from: 0.84 })),
  ];
}

function AvailabilityStatus({ availability }) {
  if (availability.status === AVAILABILITY_STATUS.SOLD_OUT) return <span className={styles.statusSold}>Sold out</span>;
  if (availability.status === AVAILABILITY_STATUS.LOW) {
    return <span className={styles.statusLow}>{availability.count === 1 ? "Only 1 left" : `Only ${availability.count} left`}</span>;
  }
  if (availability.status === AVAILABILITY_STATUS.AVAILABLE) {
    return <span className={styles.statusOpen}>{availability.count === null ? "Available" : `${availability.count} available`}</span>;
  }
        ...
        {units.map((unit, index) => {
          ...
          return (
            <article
              ...
              className={[styles.card, open ? styles.open : "", isClimate ? styles.climate : styles.driveup, availability.canRent ? "" : styles.soldOut].filter(Boolean).join(" ")}
            >
              <div className={styles.panel}>
                <p className={styles.typeBadge}>{isClimate ? "❄ Climate-Controlled" : "Drive-Up • Non-Climate"}</p>
                <div className={styles.identity}>
                  <h3>{unit.size}</h3>
                  <p className={styles.meta}><b>{unit.sqft} sq ft</b><span>{unit.label}</span></p>
                </div>
                <div className={styles.priceRow}>
                  <p className={styles.price}><small>Starting at</small><strong>{money(unit.fromPrice)}<span>/mo</span></strong></p>
                  <p className={styles.status}><AvailabilityStatus availability={availability} />{unit.promotion ? <b className={styles.promo}>{unit.promotion}</b> : null}</p>
                </div>
                ...
                {availability.canRent ? (
                  <div className={styles.actions}>
                    <a className={styles.moveIn} href={resolveCheckoutHref(unit)}>Move In <span aria-hidden="true">→</span></a>
                    <a className={styles.call} href={PHONE_LINK}>Call Us</a>
                  </div>
                ) : (
                  <div className={styles.unavailable}>
                    <p><strong>{unit.size} is currently unavailable.</strong></p>
                    {alternative ? (
                      <>
                        <p className={styles.alternative}>Closest available option: <b>{alternative.size}</b> — from {money(alternative.fromPrice)}/mo</p>
                        <a className={styles.moveIn} href={`#${anchorPrefix}-${alternative.slug}`}>See {alternative.size} <span aria-hidden="true">→</span></a>
                      </>
                    ) : (
                      <>
                        <p className={styles.alternative}>Call and we will help you find the closest fit.</p>
                        <a className={styles.moveIn} href={PHONE_LINK}>Call Us <span aria-hidden="true">→</span></a>
                      </>
                    )}
                  </div>
                )}
              ...
              <div className={styles.visual}>
                <button
                  ...
                  aria-expanded={open}
                  aria-label={open ? `Hide the ${unit.size} ${typeLabel} example` : `See what fits in a ${unit.size} ${typeLabel} unit`}
                  onClick={() => toggleUnit(unit.id)}
                >
                  <span className={styles.scene} aria-hidden="true">
                    <span className={styles.unit}>
                      <span className={styles.fascia}><i>{isClimate ? "Indoor" : "Drive-Up"}</i></span>
                      <span className={styles.mouth}>
                        <span className={styles.room}>
                          <i className={styles.roomBack} /><i className={styles.roomCeiling} /><i className={styles.roomLeft} /><i className={styles.roomRight} /><i className={styles.roomFloor} /><i className={styles.roomGlow} />
                        </span>
